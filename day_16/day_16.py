...
def problem_a():
    ...
    binary = hex_to_4_digit_binary(hexadecimal)
    _, versions, _ = read_packet(binary, 0)
...
def problem_b():
    ...
    binary = hex_to_4_digit_binary(hexadecimal)
    _, versions, value = read_packet(binary, 0)

    print("Result:", value)


def hex_to_4_digit_binary(hexadecimal):
    binary = ''
...
def read_packet(binary, index):
    ...
    if binary[index + 3:index + 6] == '100':
        index, version, value = read_value_packet(binary, index)
        versions.append(version)
    else:
        index, new_versions, value = read_operator_packet(binary, index)
        versions = versions + new_versions
    return index, versions, value


def read_operator_packet(binary, index):
    versions = []
    values = []
    operation = int(binary[index + 3:index + 6], 2)
    versions.append(int(binary[index:index + 3], 2))
    index += 6

    if binary[index] == '0':
        ## Read subpackets based on length
        index += 1
        ...
        while index < end:
            index, new_versions, value = read_packet(binary, index)
            versions = versions + new_versions
            values.append(value)
    else:
        ## Read subpackets based on amount
        index += 1
...
def read_value_packet(binary, index):
    ...
    while not last_packet:
        binary_value += binary[index + 1:index + 5]
        if binary[index] == '0':
            last_packet = True
        index += 5
        length += 5
